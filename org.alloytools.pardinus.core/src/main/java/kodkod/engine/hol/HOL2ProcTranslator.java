...
public class HOL2ProcTranslator extends AbstractReplacer {
    ...
    class HistoryAspect extends AspectReturnVisitor<Expression, Formula, Decls, IntExpression> {
        protected HistoryAspect()          { super(HOL2ProcTranslator.this); }
        protected <T> T end(Node n, T ans) { stack.pop(); skolemizable.pop(); return ans; }
        protected void start(Node n)       {
            ...
            if (!skolemizableSoFar) {
                ...
                if ((n instanceof BinaryFormula && ((BinaryFormula) n).op() == FormulaOperator.AND) ||
                    (n instanceof NaryFormula && ((NaryFormula) n).op() == FormulaOperator.AND) ||
                    (n instanceof QuantifiedFormula && ((QuantifiedFormula)n).quantifier() == Quantifier.SOME))
        ...
        @Override
        public Expression visit(TempExpression temporalExpr) {
            // TODO Auto-generated method stub
            return null;
        }
    }
    ...
    public Formula visit(QuantifiedFormula qf) {
        for (Decl decl : qf.decls()) {
            if (decl.multiplicity() != Multiplicity.ONE || decl.variable().isVariable()) {
                if (!isSkolemizableUpToNow()) throw new HigherOrderDeclException(decl);
    ...
    private boolean isSkolemizableUpToNow() {
        if (skolemizable.size() <= 1) return true;
