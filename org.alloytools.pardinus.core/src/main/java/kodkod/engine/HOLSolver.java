...
public final class HOLSolver implements KodkodSolver<PardinusBounds, ExtendedOptions>, TemporalSolver<ExtendedOptions> {
    private final ExtendedOptions options;
    private HOLTranslation translation;
    private Boolean outcome;

    private HOLSolver() {
        this(new ExtendedOptions());
    }
     ...
     */
    HOLSolver(ExtendedOptions options) {
        this.options = options;
        this.outcome = null;
    }

    public static HOLSolver solver() {
        return solver(new ExtendedOptions());
    }
     ...
     */
    public static HOLSolver solver(ExtendedOptions options) {
     ...
     */
    public Solution solve(Formula f, PardinusBounds b) throws HigherOrderDeclException, UnboundLeafException, AbortedException {
        ...
        try {
        	if (true) {
	            ...
	            translation = HOLTranslator.proc2transl(proc, options, null, new HashMap<>());
	            final long endTransl = System.currentTimeMillis();
	            return toSolution(endProcTransl - startProcTransl, endTransl - startTransl, translation);
        	} else {
	        	long totProcTransl = 0;
	        	long totTransl = 0;
	        	Solution sol = null;
	        	boolean sat = false;
	        	for (int k = options.minTraceLength(); k <= options.maxTraceLength() && !sat; k++) {
		            final long startProcTransl = System.currentTimeMillis();
		            
		            TemporalHOLTranslation trans = TemporalTranslator.translate2proc(f, b, options, k);
		            final long endProcTransl = System.currentTimeMillis();
		
		            final long startTransl = System.currentTimeMillis();
		            translation = HOLTranslator.proc2transl(trans.proc, options, null, new HashMap<>());
		            final long endTransl = System.currentTimeMillis();
		            totProcTransl += endProcTransl - startProcTransl;
		            totTransl += endTransl - startTransl;
		            sol = toSolution(totProcTransl, totTransl, translation);
		            sat = sol.sat();
	        	}
	            return sol;
            }
    ...
    private Solution toSolution(final long procTranslTime, final long translTime, final HOLTranslation translation) {
    	Solution sol = null;
    	if (true)
    		sol = Solver.solveTranslation(translTime, translation);
    	else
    		sol = TemporalPardinusSolver.solveTranslation(translTime, translation);
    ...
    @Override
    public Iterator<Solution> solveAll(final Formula formula, final PardinusBounds bounds) throws HigherOrderDeclException,
            UnboundLeafException, AbortedException {
        return new Iterator<Solution>() {
            private Solution lastSol = null;
            @Override public boolean hasNext() { return lastSol == null || lastSol.sat(); }
            @Override public Solution next()   {
                if (!hasNext()) throw new NoSuchElementException();
                lastSol = (lastSol == null) ? solve(formula, bounds) : solveNext();
                return lastSol;
            }
            @Override public void remove() { throw new IllegalStateException("can't remove solution"); }
        };
    }
     ...
     */
    public ExtendedOptions options() { return options; }
