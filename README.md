This project is a fork of [Alloy](https://github.com/alloytools/org.alloytools.alloy), extending it with support for the verification of hyperproperties.

It relies on an extended version of the [electrod](https://github.com/hpacheco/electrod) model finder to interface with SMV-based model checkers, which must be available through the `PATH` environment variable (a pre-compiled version is available for darwin/arm64 in [org.alloytools.kodkod.nativesat](org.alloytools.kodkod.nativesat)). 
